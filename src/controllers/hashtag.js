...
exports.getHashtagByCategory = async (req, res) => {
  ...
  if (!hashtag) {
    res.status(404).json({ message: `The cateogry of ${specificCategory} does not exist` })
  } else {
    res.status(200).json(hashtag);
  }
}
...
exports.getSpecificHashtagsByUserId = async (req, res) => {
  const userId = req.params.id;
  const specificCategory = req.params.category;
  const hashtag = await Hashtag.findAll({ where: { category: specificCategory, userId: userId }});
